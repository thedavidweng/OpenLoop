...
| 阶段 | 内容 | 依赖 |
|------|------|------|
| **M0** | 契约与文档：事件 schema、退出码、`backend` 子命令树 | 无 |
| **M1** | 后端归属模型 + 终止策略（孤儿进程治理） | M0 |
| **M2** | 取消生成与后端关停联动 + 跨进程取消 | M1 |
| **M3** | 统一遥测输出（人类 / JSON / NDJSON）与 `doctor`/`status` | M0、部分 M1 |
| **M4** | CLI 子命令补齐（对齐 Tauri） | M1–M3 并行可规划接口 |
| **M5** | 测试、发布说明、迁移指南 | 全部 |
