...
openloop run "cinematic strings" --ndjson
```
...
- **Model variants** — Two profiles: Base (faster, less memory) and Turbo (higher quality)
- **NDJSON streaming** — Machine-readable progress output for agent pipelines
- **9 CLI commands** — `run`, `setup`, `list`, `models`, `pull`, `ps`, `delete`, `clear`, `stop`
...
| Storage | ~5 GB for models and backend |
