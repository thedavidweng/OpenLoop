...
const openLoopCargoLockPackage = /(\[\[package\]\]\nname = "openloop"\nversion = ")([^"]+)(")/;
if (!openLoopCargoLockPackage.test(cargoLockContent)) {
  throw new Error("Could not find openloop package entry in src-tauri/Cargo.lock");
}

const nextCargoLockContent = cargoLockContent.replace(openLoopCargoLockPackage, `$1${version}$3`);
