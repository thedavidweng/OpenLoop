...
mod tests {
    ...
    fn parse_backend_status() {
        ...
        match cli.command {
            Commands::Backend { command: BackendCommand::Status } => {}
    ...
    fn parse_backend_logs_with_open() {
        ...
        match cli.command {
            Commands::Backend { command: BackendCommand::Logs { open } } => assert!(open),
    ...
    fn parse_backend_all_subcommands() {
        for sub in ["status", "start", "stop", "restart", "clear-cache", "provision", "update"] {
    ...
    fn parse_models_download_variant() {
        ...
        match cli.command {
            Commands::Models { command: Some(ModelsCommand::Download { variant }) } => {
    ...
    fn parse_models_delete_variant() {
        ...
        match cli.command {
            Commands::Models { command: Some(ModelsCommand::Delete { variant }) } => {
    ...
    fn parse_models_delete_all_with_yes() {
        ...
        match cli.command {
            Commands::Models { command: Some(ModelsCommand::DeleteAll { yes }) } => assert!(yes),
    ...
    fn parse_settings_set_key_value() {
        let cli = Cli::try_parse_from(["openloop", "settings", "set", "modelVariant", "pro"]).unwrap();
        match cli.command {
            Commands::Settings { command: Some(SettingsCommand::Set { key, value }) } => {
    ...
    fn parse_settings_reset_with_yes() {
        ...
        match cli.command {
            Commands::Settings { command: Some(SettingsCommand::Reset { yes }) } => assert!(yes),
    ...
    fn parse_generation_list() {
        ...
        match cli.command {
            Commands::Generation { command: GenerationCommand::List } => {}
    ...
    fn parse_generation_cancel_with_id() {
        ...
        match cli.command {
            Commands::Generation { command: GenerationCommand::Cancel { id, kill_backend } } => {
    ...
    fn parse_generation_cancel_all() {
        ...
        match cli.command {
            Commands::Generation { command: GenerationCommand::Cancel { id, .. } } => {
                assert!(id.is_none());
            }
            _ => panic!("expected Generation::Cancel"),
        }
    ...
    fn parse_generation_cancel_kill_backend() {
        let cli = Cli::try_parse_from(["openloop", "generation", "cancel", "--kill-backend"]).unwrap();
        match cli.command {
            Commands::Generation { command: GenerationCommand::Cancel { kill_backend, .. } } => {
                assert!(kill_backend);
            }
            _ => panic!("expected Generation::Cancel"),
        }
    ...
    fn parse_pull_with_mirror() {
        let cli = Cli::try_parse_from(["openloop", "pull", "lite", "--mirror", "https://mirror.example.com"]).unwrap();
        match cli.command {
            Commands::Pull(args) => {
    ...
    fn parse_files_reveal() {
        ...
        match cli.command {
            Commands::Files { command: FilesCommand::Reveal { path } } => {
    ...
    fn parse_files_copy() {
        ...
        match cli.command {
            Commands::Files { command: FilesCommand::Copy { src, dst } } => {
    ...
    fn parse_files_read_audio_with_output() {
        let cli = Cli::try_parse_from(["openloop", "files", "read-audio", "abc123", "--output", "-"]).unwrap();
        match cli.command {
            Commands::Files { command: FilesCommand::ReadAudio { id, output } } => {
                assert_eq!(id, "abc123");
    ...
    fn parse_files_unlink_with_keep_record() {
        let cli = Cli::try_parse_from(["openloop", "files", "unlink", "abc123", "--keep-record"]).unwrap();
        match cli.command {
            Commands::Files { command: FilesCommand::Unlink { id, keep_record } } => {
                assert_eq!(id, "abc123");
                assert!(keep_record);
            }
    ...
    fn parse_setup_flags() {
        let cli = Cli::try_parse_from(["openloop", "setup", "--model", "turbo", "--duration", "60"]).unwrap();
        match cli.command {
            Commands::Setup(args) => {
                assert_eq!(args.model.as_deref(), Some("turbo"));
                assert_eq!(args.duration, Some(60.0));
    ...
    fn model_variant_arg_converts_to_model_variant() {
        assert_eq!(ModelVariant::from(ModelVariantArg::Lite), ModelVariant::Lite);
        assert_eq!(ModelVariant::from(ModelVariantArg::Turbo), ModelVariant::Turbo);
