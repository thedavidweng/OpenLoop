...
export function useWindowShellState(sidebarWidth: number): WindowShellState {
  const platform = getShortcutPlatform();
  const resolvedInitialState = useMemo(
    () => resolveWindowShellState(platform, { sidebarWidth }),
  ...
  useEffect(() => {
    ...
    getWindowShellStateSnapshot()
      .then((snapshot) => {
        if (cancelled) return;
        setNativeState(
          resolveWindowShellState(platform, {
  ...
  }, [platform, shouldHydrateNativeSnapshot, sidebarWidth]);
